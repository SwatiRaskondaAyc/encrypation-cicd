...
const NewsSheet = ({ newsData, onDateSelect }) => {
  ...
  const handleDateChange = (e) => {
    ...
    setLocalLoading(true);
    const res = onDateSelect(val || null);
    if (res?.finally) {
  ...
  const filteredArticles = useMemo(() => {
    ...
    if (selectedSymbols.length > 0) {
      list = list.filter((a) =>
        (a.matched_symbols || []).some((m) => selectedSymbols.includes(m.symbol))
      );
    }

    list.sort(
      (a, b) =>
        parsePublishedDate(b.published_date || b.date) -
        parsePublishedDate(a.published_date || a.date)
    );
  ...
  const comingSoon = (!newsData || apiHasNoNews) && !localLoading;
  const filterEmpty =
    newsData &&
    newsData.articles?.length > 0 &&
  ...
  return (
    <div className="min-h-[500px] space-y-6">
      ...
      {/* SIMPLE LOADER */}
      {localLoading && (
        <div className="absolute inset-0 z-40 bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm flex items-center justify-center rounded-xl">
          <Loader2 className="w-10 h-10 animate-spin text-emerald-600 dark:text-emerald-400" />
        </div>
      )}

      {/* CLEAN HEADER */}
      <div className="bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-xl shadow-sm p-5">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">News Feed</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          {dateFilter ? `Filtered for ${dateFilter}` : "Latest market updates"}
        </p>
        ...
        {availableSymbols.length > 0 && (
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              onClick={setAllSymbols}
              className={`px-4 py-2 text-sm font-medium rounded-lg transition ${selectedSymbols.length === availableSymbols.length
            ...
            {availableSymbols.map((sym) => (
              <button
                key={sym}
                onClick={() => toggleSymbol(sym)}
                className={`px-4 py-2 text-sm font-medium rounded-lg transition ${selectedSymbols.includes(sym)
      ...
      {/* SIMPLE COMING SOON */}
      {comingSoon && (
        <div className="text-center py-28 bg-gray-50 dark:bg-slate-800/50 border border-dashed border-gray-300 dark:border-slate-700 rounded-xl">
          <div className="text-5xl mb-4">📰</div>
          <p className="text-xl font-semibold text-gray-800 dark:text-gray-100">
            News is on the way!
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-3">
            Stay tuned — fresh updates coming soon.
          </p>
        </div>
      )}
      ...
      {/* NO RESULTS */}
      {filterEmpty && (
        <div className="text-center py-20 text-gray-500 dark:text-gray-400 border border-dashed border-gray-300 dark:border-slate-700 rounded-xl bg-gray-50 dark:bg-slate-800/50">
          No news found for the selected filters.
        </div>
      )}

      {/* NEWS GRID */}
      {!comingSoon && !filterEmpty && (
