...
const MetricsCard = ({ ratioMetrics, currentHoldings, isExporting, className = "" }) => {
  ...
  const getColor = (val, type = "high_good") => {
    ...
    if (type === "high_good") {
      if (val > 2) return "text-emerald-700 dark:text-emerald-400";
      if (val > 1) return "text-emerald-600 dark:text-emerald-500";
      if (val > 0) return "text-blue-600 dark:text-blue-400";
      return "text-rose-600 dark:text-rose-400";
    }
    if (type === "low_good") {
      if (val < 0.15) return "text-emerald-700 dark:text-emerald-400";
      if (val < 0.25) return "text-blue-600 dark:text-blue-400";
  ...
  const getBgColor = (val) => {
    if (val === null || val === undefined) return "bg-gray-50 dark:bg-slate-800/50";
    if (val > 1) return "bg-emerald-50 dark:bg-emerald-900/20";
    if (val > 0) return "bg-blue-50 dark:bg-blue-900/20";
    if (val < 0) return "bg-rose-50 dark:bg-rose-900/20";
    return "bg-gray-50 dark:bg-slate-800/50";
  };

  const getBorderColor = (val) => {
    if (val === null || val === undefined) return "border-gray-200 dark:border-slate-700";
    if (val > 1) return "border-emerald-200 dark:border-emerald-800";
    if (val > 0) return "border-blue-200 dark:border-blue-800";
    if (val < 0) return "border-rose-200 dark:border-rose-800";
  ...
  return (
    <>
      <div
        ...
        )} ${getBorderColor(pMetrics?.sharpe_ratio)} hover:shadow-lg ${className}`}
      >
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          {/* Sharpe Ratio */}
          <div className="p-4 bg-white/70 dark:bg-slate-800/70 rounded-lg border border-gray-200 dark:border-slate-700 flex flex-col">
            <span
        ...
        <button
          ...
          className="w-full py-2 flex items-center justify-center gap-2 text-sm text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-md font-medium transition-colors"
        >
          <Eye size={16} />
          See All Metrics
        </button>
      ...
      {showDetails && (
        <div className={isExporting ? "pdf-section-container mt-12 bg-white" : "fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"} data-pdf-section="metrics-card-details">
          <div className={isExporting ? "bg-white rounded-none border-0 w-full relative flex flex-col" : "bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-7xl w-full max-h-[95vh] overflow-hidden relative flex flex-col border dark:border-slate-700"}>
            {/* Header */}
            <div className="bg-slate-900 dark:bg-slate-950 px-6 py-4 flex items-center justify-between shrink-0 border-b dark:border-slate-800">
              <div className="flex items-center gap-4">
                ...
                {!isExporting && (
                  <button
                    ...
                    className="bg-slate-800 hover:bg-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800 text-blue-300 dark:text-blue-400 px-3 py-1 rounded-full text-xs font-semibold transition-colors flex items-center gap-1 border border-slate-700 dark:border-slate-800"
                  >
            ...
            {/* Glossary Full Screen Overlay - Always visible for PDF if isExporting is true */}
            {showHelp && (
              <div className={`${isExporting ? 'relative' : 'absolute inset-0 z-50'} bg-white dark:bg-slate-900 flex flex-col`}>
                {/* Overlay Header */}
                <div className="bg-slate-900 dark:bg-slate-950 px-6 py-4 flex items-center justify-between shrink-0 border-b dark:border-slate-800">
                  <div>
                  ...
                  {!isExporting && (
                    <button
                      ...
                      className="text-slate-400 hover:text-white hover:bg-slate-800 dark:hover:bg-slate-800 rounded-full p-2 transition-colors"
                    >
                      <X size={20} />
                    </button>
                  )}
                  ...
                  <div className="max-w-6xl mx-auto">
                    <div className="flex flex-col gap-6">
                      {GLOSSARY_CONTENT.map((item, idx) => (
                        <div
                          key={idx}
                          className="bg-white dark:bg-slate-800 p-8 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow"
                        >
                          <div className="flex items-start justify-between mb-4">
                            <h5 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center text-blue-700 dark:text-blue-400 text-lg font-bold">
                                {idx + 1}
                              </div>
                              {item.title}
                            </h5>
                            {item.thresholds && (
                              <span
                                className="text-xs font-mono text-blue-800 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 px-3 py-1.5 rounded border border-blue-100 dark:border-blue-800"
                          ...
                          {/* Concept */}
                          <p className="text-slate-700 dark:text-slate-300 mb-4 leading-relaxed border-b border-slate-100 dark:border-slate-700 pb-4">
                            {item.concept}
                          </p>

                          {/* Example Section - NEW */}
                          {item.example && (
                            <div className="mb-6 p-4 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg border-l-4 border-amber-400 dark:border-amber-600">
                          ...
                          <div className={`grid gap-8 ${isExporting ? 'grid-cols-1' : 'md:grid-cols-2'}`}>
                            <div>
                              ...
                              <div className="bg-slate-900 dark:bg-black text-blue-300 dark:text-blue-400 p-5 rounded-lg font-mono text-center mb-4 shadow-inner border border-slate-800 dark:border-slate-700">
                                {/* Render HTML formula */}
                                <div
                                  dangerouslySetInnerHTML={{
                                    __html: item.formula,
                              ...
                              {item.variables && item.variables.length > 0 && (
                                <div className="space-y-3">
                                  ...
                                  {item.variables.map((v, vIdx) => (
                                    <div
                                      key={vIdx}
                                      className="text-sm bg-slate-50 dark:bg-slate-900/50 p-3 rounded border border-slate-200 dark:border-slate-700"
                                    >
                                      <div
                                        className="font-bold text-slate-700 dark:text-slate-200 font-mono mb-1"
                                        dangerouslySetInnerHTML={{
                                          __html: v.name,
                                        }}
                                      />
                                      <div className="text-slate-600 dark:text-slate-400 font-semibold text-xs">
                                        {v.desc}
                                      </div>
                            ...
                            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg p-6 border border-blue-100 dark:border-blue-800">
                              <p className="text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider mb-3 flex items-center gap-1">
            ...
            {/* Benchmark Selector */}
            <div className="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-b border-gray-200 dark:border-slate-700 shrink-0 flex items-center gap-6">
              <div>
                ...
                <select
                  ...
                  disabled={loading}
                  className="px-3 py-1.5 border border-gray-300 dark:border-slate-700 rounded text-sm bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[200px]"
                >
            ...
            {/* Content */}
            <div className="px-6 py-6 overflow-y-auto grow bg-gray-50/50 dark:bg-slate-900/30">
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead className="bg-gray-50 dark:bg-slate-900">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider sticky left-0 bg-gray-50 dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 z-10 w-32">
                          Symbol
                        </th>
                        ...
                        <th className="px-4 py-3 bg-blue-50/50 dark:bg-blue-900/10">
                          <ColumnHeaderWithInfo
                            label="Active Ret."
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-slate-700 text-sm">
                      {Object.entries(individual).length > 0 ? (
                        Object.entries(individual).map(([symbol, data]) => (
                          <tr key={symbol} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 group">
                            <td className="px-4 py-3 font-bold text-gray-900 dark:text-gray-100 sticky left-0 bg-white dark:bg-slate-800 group-hover:bg-gray-50 dark:group-hover:bg-slate-700 border-r border-gray-100 dark:border-slate-700">
                              {symbol}
                            </td>
                            ...
                            {/* Active Return */}
                            <td
                              className={`px-4 py-3 font-medium bg-blue-50/30 ${data?.active_return > 0
                                ? "text-green-600"
                                : "text-red-600"
                                }`}
                            >
            ...
            <div className="px-6 py-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 shrink-0">
              <button
                onClick={() => setShowDetails(false)}
                className="w-full px-4 py-2 bg-slate-800 dark:bg-slate-900 text-white dark:text-gray-100 rounded-md font-medium text-sm hover:bg-slate-700 dark:hover:bg-slate-800 transition-colors shadow-sm"
              >
            ...
            {tooltipData && (
              ...
              >
                <div className="font-bold text-gray-900 dark:text-gray-100 mb-2 border-b dark:border-slate-700 pb-1 font-serif text-lg capitalize">
                ...
                <div className="mb-4 text-center p-3 bg-gray-50 dark:bg-slate-900/50 rounded border border-gray-100 dark:border-slate-700">
                  {tooltipData.metricType === "sharpe" && (
                    <div className="font-serif italic text-base">
                      ...
                      <div className="border-t border-black dark:border-gray-600 w-full my-1"></div>
                  ...
                  {tooltipData.metricType === "downside" && (
                    <div className="text-sm font-sans text-gray-700 dark:text-gray-300">
                      Standard Deviation of negative returns only.
                      <div className="font-mono font-bold mt-2 text-red-700 dark:text-red-400">
                        {formatPercent(tooltipData.data.downside_volatility)}
                      </div>
                    </div>
                  )}
