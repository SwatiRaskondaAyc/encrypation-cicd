...
const RollingGraphModal = ({ isOpen, onClose, title, data, color }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Rolling 30-day analysis</p>
          </div>
          <button
            onClick={onClose}
...
const MetricCube = ({
  ...
  return (
    <div
      className={`relative group p-5 rounded-2xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-1 bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700`}
    >
      {/* Background Blob for aesthetic */}
      <div
        className={`absolute top-0 right-0 w-24 h-24 rounded-full opacity-10 dark:opacity-20 blur-2xl -mr-6 -mt-6 transition-colors ${colorClass}`}
      ></div>
      ...
      <div className="flex justify-between items-start mb-3 relative z-10">
        ...
        {rollingData && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1"
          >
...
const PortfolioMetrics = ({ metrics, onBack, isLoading }) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 mb-6 flex justify-center">
  ...
  const handleOpenGraph = (title, data, textColorClass) => {
    ...
    const colorMap = {
      ...
      "text-violet-600 dark:text-violet-400": "#a78bfa",
      "text-emerald-600 dark:text-emerald-400": "#34d399",
      "text-rose-600 dark:text-rose-400": "#fb7185",
      "text-amber-600 dark:text-amber-400": "#fbbf24",
    };
    setGraphState({
      isOpen: true,
      title,
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300">
      ...
      <div className="flex items-center justify-between gap-4 mb-8">
        ...
        <button
          onClick={onBack}
          className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 text-gray-600 dark:text-gray-300 transition-all shadow-sm text-sm"
        >
