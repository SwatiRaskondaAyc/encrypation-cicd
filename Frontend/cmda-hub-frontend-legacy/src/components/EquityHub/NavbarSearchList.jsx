...
// function NavbarSearchList({ onSelect, showFilters = false }) {
...
//   // Correct API Config
//   // const API_URL = `${config.API_BASE}/search/options`;
...
//     return {
//       uniqueHouses: Array.from(houses.entries()).map(([code, label]) => ({ code, label })).sort((a, b) => a.label.localeCompare(b.label)),
//       sectorMap: sectors,
...
//       // 1. Search Text
//       const matchesSearch = !s || c.symbol.toLowerCase().includes(s) || c.companyName.toLowerCase().includes(s);
//       if (!matchesSearch) return false;

//       // 2. House Filter
//       if (selectedHouses.size > 0 && !selectedHouses.has(c.houseCode)) return false;
...
//   }, [companies, search, selectedHouses, selectedSectors, selectedIndustries, priceRange]);


//   // ✅ API Integration (Final Correct Version)
//   const fetchData = useCallback(async (searchTerm) => {
//     // Fetch all if search is empty to populate filters? Or just fetch on mount?
//     // For now, let's fetch on mount or empty search to get all data for filters
//     // if (!searchTerm.trim()) {
//     //   setCompanies([]);
//     //   return;
//     // }
...
//     try {
//       const response = await fetch(`${API_BASE}/equity-insights/search/options`, {
//         method: "GET",
//         headers: {
...
//         const rows = result.data.Symbol.map((symbol, i) => ({
//           symbol,
//           companyName: result.data.Company_ShortName[i],
//           fincode: result.data.FINCODE[i],
//           equityPA_Flag: result.data.EquityPA_Flag[i],
//           sector: result.data.Sector[i],
//           industry: result.data.Industry_ShortName[i],
//           house: result.data.House[i],
//           // New Fields
//           indCode: result.data.IND_CODE[i],
//           sectorCode: result.data.Sector_code[i],
//           houseCode: result.data.HSE_CODE[i],
//           price: result.data.Price[i]
//         }));
...
//       }
//     } catch (err) {
//       setError("Cannot reach API. Check server or CORS.");
//       setCompanies([]);
//     } finally {
//       setLoading(false);
//     }
...
//         onKeyDown={handleKeyDown}
//         placeholder="Search by symbol or company name..."
//         className="w-full pl-12 pr-12  bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-xl text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 font-medium text-base shadow-sm hover:shadow-md"
//         /> */}
//         <div className="relative group">
//   <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
//     <svg 
//       className="h-5 w-5 text-gray-400 dark:text-slate-400 group-focus-within:text-cyan-500 transition-colors duration-200" 
//       fill="none" 
//       viewBox="0 0 24 24" 
//       stroke="currentColor"
//     >
...
//     className="w-full pl-12 pr-12 py-3.5 bg-white dark:bg-slate-800 border-2 border-gray-200 dark:border-slate-700 rounded-xl text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-slate-400 focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-100 dark:focus:ring-cyan-900/30 transition-all duration-200 font-medium text-base shadow-sm hover:border-gray-300 dark:hover:border-slate-600"
//   />
...
//     >
//       <svg 
//         className="h-5 w-5 text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-slate-300 transition-colors duration-200" 
//         fill="none" 
//         viewBox="0 0 24 24" 
//         stroke="currentColor"
//       >
...
//     {(selectedHouses.size > 0 || selectedSectors.size > 0 || selectedIndustries.size > 0 || priceRange[0] > 0 || priceRange[1] < maxPrice) && (
//       <span className="ml-1.5 bg-white text-cyan-600 dark:bg-cyan-500 dark:text-white text-xs font-bold px-2 py-0.5 rounded-full">
//         {selectedHouses.size + selectedSectors.size + selectedIndustries.size + (priceRange[0] > 0 || priceRange[1] < maxPrice ? 1 : 0)}
//       </span>
//     )}
//   </button> */}
//   {showFilters && (
...
//                   {selectedHouses.size > 0 && <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">House ({selectedHouses.size})</span>}
//                   {(selectedSectors.size > 0 || selectedIndustries.size > 0) && <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Sector/Ind</span>}
//                   {(priceRange[0] > 0 || priceRange[1] < maxPrice) && <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded">Price</span>}
//                 </div>
...
//                         .map(h => (
//                           <label key={h.code} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
//                             <input
//                               type="checkbox"
//                               checked={selectedHouses.has(h.code)}
//                               onChange={() => setSelectedHouses(prev => toggleSet(prev, h.code))}
...
//                   {Array.from(sectorMap.entries()).sort((a, b) => a[1].label.localeCompare(b[1].label)).map(([code, data]) => (
//                     <div key={code} className="border rounded-md overflow-hidden">
//                       <div className="flex items-center bg-gray-50 p-2">
//                         <button onClick={() => toggleExpandSector(code)} className="p-1 hover:bg-gray-200 rounded mr-2">
//                           {expandedSectors.has(code) ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
//                         </button>
...
//                           {Array.from(data.industries.entries()).map(([indCode, indLabel]) => (
//                             <label key={indCode} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
//                               <input
//                                 type="checkbox"
...
//                                 className="rounded text-blue-600 focus:ring-blue-500"
//                               />
//                               <span className="text-sm text-gray-600">{indLabel}</span>
//                             </label>
//                           ))}
//                         </div>
//                       )}
...
//                   className={`px-3 py-2 cursor-pointer border-b border-gray-100 hover:bg-blue-50 transition-all duration-200 ${index === selectedIndex ? "bg-blue-100" : ""
//                     }`}
//                 >
//                   <div className="flex flex-col gap-0.5">
//                     {/* Top Row: Symbol & Company Name */}
//                     <div className="flex justify-between items-baseline">
//                       <span className="font-bold text-gray-900 text-base">{company.symbol}</span>
//                       <span className="text-sm font-medium text-gray-600 truncate ml-4">{company.companyName}</span>
//                     </div>
...
// import { Filter, ChevronRight, ChevronDown, X, Search, XCircle } from "lucide-react";
...
//     {/* Larger Search Input - More height and padding */}
...
//         onKeyDown={handleKeyDown}
//         placeholder="Search symbol or company name..."
//         className="w-full py-4 pl-14 pr-12 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-3xl text-gray-900 dark:text-white placeholder-gray-500 text-base font-medium focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all duration-300 shadow-inner hover:shadow-md"
//       />
...
//       <div
//         ref={dropdownRef}
//         className="absolute w-full top-full left-0 right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl shadow-2xl overflow-hidden z-50 max-h-[70vh] overflow-y-auto"
//       >
//         <div className="py-4">
//           {filteredResults.map((company, index) => (
//             <div
...
//               onClick={() => handleSelect(company)}
//               className={`px-7 py-5 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 ${
//                 index === selectedIndex ? "bg-gray-50 dark:bg-gray-700" : ""
//               }`}
//             >
//               <div className="flex justify-between items-center">
//                 {/* Left: Symbol, Name, Tags *
...
//                     <span className="bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300 px-4 py-1.5 rounded-full text-sm font-medium">
...
//                   {company.price ? (
//                     <div className="font-semibold text-xl text-green-600 dark:text-green-400">
//                       ₹{company.price.toFixed(2)}
//                     </div>
//                   ) : (
...
//           </div>
//         )}
//     {/* No Results - Larger container */}
//     {showDropdown && search && filteredResults.length === 0 && !loading && (
//       <div className="absolute top-full left-0 right-0 mt-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl shadow-2xl p-10 text-center text-gray-500 z-50">
//         <p className="text-base">No results found</p>
//       </div>
//     )}

//     {error && (
//       <p className="mt-4 p-4 bg-red-50 text-red-800 rounded-2xl text-sm">{error}</p>
//     )}
//   </div>
// );
// }
...
function NavbarSearchList({ onSelect, showFilters = false }) {
  ...
  // Using centralized API service instead of VITE_URL
  ...
  //       `${API_BASE}/equity-insights/search/options`
  //     );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
     ...
     toast.warn("Search is temporarily unavailable. Please try again later.");
    ...
    <div className="relative w-full">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full py-3 pl-8 pr-10 border rounded-full bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-200"
        />
          ...
          {filteredResults.length > 0 ? (
            filteredResults.map((company, index) => (
              <div
                ...
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => handleSelect(company)}
                className={`px-4 py-3 cursor-pointer border-b hover:bg-blue-50 ${
