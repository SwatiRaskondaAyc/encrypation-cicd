...
const PatternMenu = ({
  ...
  const availablePatterns = Object.entries(PatternRegistry.patterns).map(
    ([id, meta]) => ({
      id: id,
      ...meta,
    }),
  ...
  const filteredPatterns = useMemo(() => {
    return availablePatterns.filter((pat) => {
      const matchesSearch =
        ...
        pat.shortName.toLowerCase().includes(searchQuery.toLowerCase());
            ...
            <div className="flex-1 overflow-y-auto px-2 space-y-1">
              {categories.map((cat) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 ${cat.className || ""} ${isActive ? "text-blue-500" : "text-gray-400"}`}
                    />
                    {cat.label}
                  </button>
                );
            ...
            <div className="flex-1 overflow-y-auto p-2">
              <div className="grid grid-cols-1 gap-1">
                {filteredPatterns.map((pat) => {
                  ...
                  const badgeClass =
                    badgeColors[pat.bias] || "bg-gray-100 text-gray-500";
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <Star
                            className={`w-4 h-4 ${favorites.some((f) => f.payload === pat.id) ? "fill-current" : ""}`}
                          />
                        </button>
                      </div>
                    </div>
