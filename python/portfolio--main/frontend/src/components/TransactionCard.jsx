...
const TransactionCard = ({ data, isParent }) => {
  ...
  const shadowClass = isSold
    ? "shadow-[0_8px_30px_rgb(251,191,36,0.15)] hover:shadow-[0_8px_30px_rgb(251,191,36,0.25)]"
    : "shadow-[0_8px_30px_rgb(59,130,246,0.15)] hover:shadow-[0_8px_30px_rgb(59,130,246,0.25)]";

  const containerClasses = `bg-white rounded-xl border border-gray-200 border-l-8 ${borderClass} ${shadowClass} transition-all duration-300 ease-out mb-4 hover:scale-[1.005] hover:-translate-y-0.5`;
    ...
    return (
      <div className="relative inline-block z-10">
        <button
          ...
          className="ml-1.5 text-gray-400 hover:text-blue-600 transition-all duration-200 cursor-help hover:scale-110"
        >
          <Info size={15} strokeWidth={2.5} />
        </button>
        {isActive && (
          <div
            className={`absolute bottom-full mb-2 w-72 bg-gray-900 text-white text-xs rounded-xl p-4 shadow-2xl ${posClass} animate-in fade-in zoom-in-95 duration-200`}
          >
            ...
            <div
              className={`absolute top-full w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-gray-900 ${arrowClass} -mt-px`}
            ></div>
          </div>
        )}
      </div>
  ...
  return (
    <div className={containerClasses}>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 pt-3 border-t border-gray-100">
          <div className="flex gap-6 md:gap-10 text-sm">
            ...
            <div>
              ...
              <p className="font-bold text-gray-800 text-base hover:text-blue-600 transition-colors cursor-pointer">
            ...
            <div>
              ...
              <p
                className={`font-bold text-base hover:text-blue-600 transition-colors cursor-pointer ${
                  isSold ? "text-gray-800" : "text-blue-600"
                }`}
              >
            ...
            {isSold ? (
              <>
                <div>
                  ...
                  <p className="font-medium text-gray-600 text-base hover:text-orange-600 transition-colors cursor-pointer">
                    {formatCost(data.total_sell_brokerage)}
                  </p>
                </div>
                <div>
                  <div className="text-xs font-bold text-gray-400 uppercase mb-0.5 flex items-center">
                    Total Sell Taxes
                    <InfoTooltip
                      id={`tax-sell-${data.id}`}
                  ...
                  <p className="font-medium text-gray-600 text-base hover:text-red-600 transition-colors cursor-pointer">
      ...
      {isParent && isOpen && data.buy_legs && (
        <div className="bg-gray-50/70 border-t-2 border-gray-100 p-3">
          <div className="space-y-2">
            {data.buy_legs.map((leg, idx) => (
              <div
                key={idx}
                className="grid grid-cols-12 items-center bg-white border border-gray-200 p-2 rounded-lg shadow-sm"
              >
                ...
                <div className="col-span-9 grid grid-cols-5 gap-2 text-[11px]">
                  <div className="text-right">
                    <span className="block font-bold text-gray-300 text-[8px] uppercase">
                      Buy Date
                    </span>
