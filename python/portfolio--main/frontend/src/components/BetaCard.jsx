...
const BetaCard = ({ betaMetrics, currentHoldings, onBenchmarkChange }) => {
  ...
  const getBetaColor = (beta) => {
    ...
    if (beta > 1.2) return "text-rose-600";
    if (beta > 0.8) return "text-amber-600";
    return "text-emerald-600";
  };

  const getBetaBgColor = (beta) => {
    if (beta === null || beta === undefined) return "bg-gray-50";
    if (beta > 1.2) return "bg-rose-50";
    if (beta > 0.8) return "bg-amber-50";
    return "bg-emerald-50";
  };

  const getBetaBorderColor = (beta) => {
    if (beta === null || beta === undefined) return "border-gray-200";
    if (beta > 1.2) return "border-rose-200";
    if (beta > 0.8) return "border-amber-200";
  ...
  const handleBenchmarkChange = async (newBenchmark) => {
    ...
    try {
      console.log("Sending API request to:", `${API_BASE}/api/calculate-beta`);
      console.log("Payload:", {
        current_holdings: currentHoldings,
        benchmark: newBenchmark,
      });

      const response = await axios.post(`${API_BASE}/api/calculate-beta`, {
        current_holdings: currentHoldings,
        benchmark: newBenchmark,
      });
    ...
    <>
      <div
        className={`p-8 rounded-xl shadow-sm border transition-all duration-200 h-full relative ${getBetaBgColor(
          portfolioBeta
        )} ${getBetaBorderColor(portfolioBeta)} hover:shadow-lg`}
      >
        ...
        <p className="text-sm text-gray-500 mb-4">vs {currentBenchmark}</p>
      ...
      {/* Modal for Beta Details */}
      {showDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[88vh] overflow-hidden relative">
            {/* Header */}
            ...
            {/* Glossary Overlay */}
            {showHelp && (
              <div className="absolute top-[72px] left-0 right-0 z-40 bg-white border-b border-gray-200 shadow-lg p-6 animate-in slide-in-from-top-2">
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    ...
                    <div>
                      ...
                      <ul className="space-y-1.5 bg-blue-50 p-3 rounded-md border border-blue-100">
            ...
            <div className="px-8 py-5 bg-gray-50 border-b border-gray-200">
              ...
              <select
                ...
                disabled={loading}
                className="w-full px-4 py-2.5 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100"
              >
                    ...
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider group relative">
                          ...
                          <div className="hidden group-hover:block absolute z-10 w-64 p-2 mt-1 text-xs font-normal normal-case bg-gray-900 text-white rounded shadow-lg">
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider group relative">
                          ...
                          <div className="hidden group-hover:block absolute z-10 w-64 p-2 mt-1 text-xs font-normal normal-case bg-gray-900 text-white rounded shadow-lg right-0">
            ...
            {tooltipData && (
              ...
              >
                <h5 className="font-bold text-gray-900 mb-2 border-b pb-1 font-serif text-lg">
                  Beta Calculation
                </h5>

                {/* LaTeX-style Formula */}
                <div className="mb-4 text-center p-2 bg-gray-50 rounded border border-gray-100">
