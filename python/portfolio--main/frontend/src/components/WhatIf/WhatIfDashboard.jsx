...
const WhatIfDashboard = ({ transactionsData }) => {
      ...
      (async () => {
        try {
          const res = await axios.post(
            "http://127.0.0.1:8000/api/what-if/enrich-holdings",
            {
              holdings: normalized,
            }
          );
  ...
  const fetchAllSectors = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get("http://127.0.0.1:8000/api/what-if/sectors");
  ...
  const handleSectorSelect = async (sectorName) => {
    ...
    try {
      const res = await axios.get(
        `http://127.0.0.1:8000/api/what-if/hierarchy/sector/${encodeURIComponent(
          sectorName
        )}`
      );
  ...
  const handleGlobalSearch = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://127.0.0.1:8000/api/what-if/sector-hierarchy/${encodeURIComponent(
          searchTerm
        )}`
      );
  ...
  const handleInitiateSwap = async (stock) => {
    ...
    if (stock.symbol === "__ADD_NEW__") {
      ...
      try {
        const res = await axios.get(
          `http://127.0.0.1:8000/api/what-if/sector-hierarchy/${stock.symbol}`
        );
        setHierarchyData(res.data);
      } catch (err) {
  ...
  const handleSimulate = async (payload) => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://127.0.0.1:8000/api/what-if/simulate",
        reqPayload
      );
  ...
  return (
    <div className="w-full h-full font-sans text-slate-800 pb-12 overflow-x-hidden selection:bg-indigo-100 selection:text-indigo-900">
      {/* HEADER */}
      <div className="relative z-10 w-full bg-transparent border-b border-indigo-50/50 py-6 mb-6">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all border border-indigo-100"
            >
            ...
            {analysisResult && (
              <button
                onClick={handleReset}
                className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 shadow-sm transition-all group"
              >
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-8">
        {/* ERROR TOAST */}
        {error && (
          <div className="mb-8 p-4 bg-rose-50/80 backdrop-blur-sm border border-rose-200 rounded-2xl text-rose-700 text-sm font-medium shadow-sm animate-in fade-in slide-in-from-top-2">
            {error}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="col-span-12 xl:col-span-7 transition-all duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1)]">
            {loading ? (
              <div className="min-h-[500px] flex flex-col items-center justify-center bg-white/60 backdrop-blur-3xl rounded-[32px] border border-white/60 shadow-2xl">
                <CandleLoader />
                <p className="mt-10 text-slate-500 font-bold tracking-wide animate-pulse">
              ...
              <div className="min-h-[500px] flex flex-col items-center justify-center bg-white/60 backdrop-blur-3xl rounded-[32px] border border-white/60 shadow-2xl p-12 text-center">
      ...
      {isHierarchyOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-12">
          ...
          />
          <div className="relative bg-white/80 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/60 w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            {/* Modal Header */}
            <div className="p-8 border-b border-indigo-50 flex flex-col bg-white/40">
              {/* Header with Search */}
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              {loadingHierarchy ? (
                  ...
                  {allSectors
                    ...
                    .map((sec) => (
                      <button
                        ...
                        className="p-6 text-left bg-white/60 border border-slate-200 rounded-2xl hover:border-indigo-400 hover:bg-white hover:shadow-lg hover:shadow-indigo-500/10 hover:text-indigo-700 transition-all group"
                      >
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                    <div className="flex items-center gap-3">
                      <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 text-xs font-bold rounded-lg uppercase tracking-wide">
                    ...
                    <button
                      ...
                      className="text-sm font-bold text-indigo-600 hover:text-indigo-800 hover:underline decoration-2 underline-offset-4"
                    >
                      Change Sector
                    </button>
                  </div>
                      ...
                      .map((ind) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {isExpanded && (
                              <div className="border-t border-slate-100 bg-slate-50/30 p-4 grid gap-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                                {stocks
                                  .filter(
                                    (peer) =>
                                      !searchTerm ||
                                  ...
                                  .map((peer) => (
                                    <button
                                      ...
                                      className="flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 hover:border-indigo-400 hover:shadow-lg hover:shadow-indigo-500/10 transition-all group text-left"
                                    >
                                      <div>
                                        <div className="font-bold text-slate-800 text-base group-hover:text-indigo-700">
                                          {peer.symbol}
                                        </div>
                ...
                <div className="text-center py-20 text-slate-400">
                  ...
                  <button
                    onClick={fetchAllSectors}
                    className="text-indigo-500 text-base font-bold mt-4 hover:underline"
                  >
                    Browse All Sectors
                  </button>
                </div>
